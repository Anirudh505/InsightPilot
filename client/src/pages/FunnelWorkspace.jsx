  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] -m-6 lg:-m-8">
      ...
      <div className="flex-1 overflow-auto bg-muted/10 p-6 lg:p-8">
        ...
        >
          {/* Empty State Handler */}
          {(!funnelData || funnelData.status === 'empty') && !isLoadingFunnel ? (
            <div className="flex flex-col items-center justify-center h-96 text-center">
              <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center mb-4">
                <Filter className="w-8 h-8 text-muted-foreground" />
              </div>
              <h2 className="text-xl font-semibold mb-2">No Funnels Found</h2>
              <p className="text-muted-foreground max-w-md">
                You haven't created any conversion funnels for this project yet. Start tracking user flows to see where drop-offs happen.
              </p>
            </div>
          ) : (
            <>
